...
if [ $SCHEMA = 'desimper' ]
then
    echo "---" > "$OUTDIR"/"90_GLOSSARY.sql"
    pg_dump service=$SERVICE --data-only --inserts --column-inserts -n $SCHEMA --no-acl --no-owner --table "$SCHEMA.nomenclature_*" -f "$OUTDIR"/90_GLOSSARY.sql
