...
def qgis_level(logging_level):
    ...
    """
    if logging_level == "CRITICAL" or logging_level == "ERROR":
...
class QgsLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        """
        try:
            QgsMessageLog.logMessage(record.getMessage(), self.plugin_name, qgis_level(record.levelname))
        except MemoryError:
            message = tr(
                "Due to memory limitations on this machine, "
                f"the plugin {self.plugin_name} can not handle the full log"
            )
            QgsMessageLog.logMessage(message, self.plugin_name, Qgis.MessageLevel.Critical)
